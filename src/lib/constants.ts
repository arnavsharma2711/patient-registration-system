...
export const defaultSQLQueries = [
  {
  ...
  },
  {
    ...
    query: `
      ...
      FROM patients
      WHERE TO_DATE(registration_date, 'YYYY-MM-DD') >= CURRENT_DATE - INTERVAL '30 days';
    `,
  },
  {
  ...
  },
  {
    ...
    query: `
      SELECT 
        ...
        COUNT(*) AS count
      FROM (
        SELECT EXTRACT(YEAR FROM AGE(TO_DATE(date_of_birth, 'YYYY-MM-DD'))) AS age
        FROM patients
  ...
  },
  {
    ...
    query: `
      SELECT 
        TO_DATE(registration_date, 'YYYY-MM-DD') AS date,
        COUNT(*) AS registrations
      FROM patients
      WHERE TO_DATE(registration_date, 'YYYY-MM-DD') >= CURRENT_DATE - INTERVAL '6 days'
      GROUP BY date
      ORDER BY date;
    `,
  },
  ...
  },
  {
    ...
    query: `
      SELECT 
        TO_CHAR(TO_DATE(registration_date, 'YYYY-MM-DD'), 'YYYY-MM') AS month,
        COUNT(*) AS registrations
      FROM patients
      WHERE TO_DATE(registration_date, 'YYYY-MM-DD') >= DATE_TRUNC('month', CURRENT_DATE) - INTERVAL '11 months'
      GROUP BY month
      ORDER BY month;
    `,
  },
  ...
  },
  {
    ...
    query: `
      ...
      FROM patients
      WHERE language_preference IS NOT NULL AND language_preference <> ''
  ...
  },
  {
    ...
    query: `
      SELECT 
        CASE 
          WHEN emergency_contact IS NULL OR emergency_contact = '' THEN 'No Emergency Contact' 
          ELSE 'Has Emergency Contact' 
        END AS emergency_contact_status,
        COUNT(*) AS count
      FROM patients
      GROUP BY emergency_contact_status;
    `,
  },
];
